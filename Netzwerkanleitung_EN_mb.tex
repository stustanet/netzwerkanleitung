...
There are two ways to use the internet. One way is to use the proxyserver. For this method, you have to configure every application, if supported. Some software doesn't support the usage of a proxy server, for example WhatsApp or League of Legends. Here, a membership is neccessary to use this software.\\
\\
\\
Membership in the StuStaNet~e.V. requires a \textbf{one-time} fee of \EUR{20} and has a number of advantages including (in addition to Internet via Proxy), full Internet over a NAT masquerader, your own e-mail address, webspace with PHP and database support\footnote{\url{http://stusta-wiki.de/Services}}. You can visit the StuStaNet~e.V. at one of our office hours, which are normally Mondays and Thursdays from 19.00 to 19.30 in the House 10, room 1002 (in the cellar, postal address: Hans-Leipelt-Straße 7, 80805 München), during the semester (during breaks only Thursdays). During public holidays there are no office hours. The exact dates can be found at \url{https://stustanet.de}. Therefore, please register first at \mbox{\url{https://reg.stustanet.de}}.
